...
;; group-picture : List ListOfPeople -> image
;; WHERE: the list is non-empty
;; RETURNS an image containing all the pictures of people in list just like 
;; a group picture
;; Examples: 
;; (group-picture (list (make-person "Nikhil" "Singh" 24 180 80) 
;; (make-person "Amanda" "Martini" 34 160 70))) => 
;; an image containing picture of Nikhil and Amanda side by side.


(define (group-picture lst)
  (cond
    [(empty? lst)  (text "" 15 "")]
    [else (beside/align 
   "bottom"
...
(define (person-image p)
   (scale (/ (person-height p) 200) human)
   )